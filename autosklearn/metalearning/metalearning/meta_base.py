...
class MetaBase(object):
    def __init__(self, configuration_space, aslib_directory, logger, meta):
        ...
        #print(aslib_directory)

        aslib_reader = aslib_simple.AlgorithmSelectionProblem(self.aslib_directory)
        self.metafeatures = aslib_reader.metafeatures #.drop(meta[0],axis=1)
        #print(type(self.metafeatures))
        self.algorithm_runs = aslib_reader.algorithm_runs
        self.configurations = aslib_reader.configurations


        #from csv to Configuration object
        #print('lib:', self.aslib_directory)
        #print('configs:',self.configurations['707'])
        configurations = dict()
    ...
    def add_dataset(self, name, metafeatures):
        ...
        #print(self.metafeatures)
        #print(self.metafeatures.index)
        if name.lower() in self.metafeatures.index:
            print('meta_base: Dataset %s already in meta-data. Removing occurence.', name.lower())
            self.logger.warning(
