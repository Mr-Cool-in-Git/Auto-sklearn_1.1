...
# My features
################################################################################
@metafeatures.define("conceptvar_sd")
class conceptvar_sd(MetaFeature):
    def _calculate(self, X, y, logger, categorical):
        return np.std(MFEConcept.ft_conceptvar(X,y),ddof=1)

@metafeatures.define("lsc")
class lsc(MetaFeature):
    def _calculate(self, X, y, logger, categorical):
        return MFEComplexity.ft_lsc(X,y)

@metafeatures.define("freq_class_sd")
class freq_class_sd(MetaFeature):
    def _calculate(self, X, y, logger, categorical):
        return np.std(MFEGeneral.ft_freq_class(X,y),ddof=1)

@metafeatures.define("n3_mean")
class n3_mean(MetaFeature):
    def _calculate(self, X, y, logger, categorical):
        return np.mean(MFEComplexity.ft_n3(X,y))

@metafeatures.define("nr_cor_attr")
class nr_cor_attr(MetaFeature):
    def _calculate(self, X, y, logger, categorical):
        return MFEStatistical.ft_nr_cor_attr(X)

@metafeatures.define("f1_mean")
class f1_mean(MetaFeature):
    def _calculate(self, X, y, logger, categorical):
        return np.mean(MFEComplexity.ft_f1(X,y))

@metafeatures.define("c2")
class c2(MetaFeature):
    def _calculate(self, X, y, logger, categorical):
        return MFEComplexity.ft_c2(X,y)

@metafeatures.define("f4_mean")
class f4_mean(MetaFeature):
    def _calculate(self, X, y, logger, categorical):
        return np.mean(MFEComplexity.ft_f4(X,y))

@metafeatures.define("n1")
class n1(MetaFeature):
    def _calculate(self, X, y, logger, categorical):
        return MFEComplexity.ft_n1(X,y)

@metafeatures.define("l1_mean")
class l1_mean(MetaFeature):
    def _calculate(self, X, y, logger, categorical):
        return np.mean(MFEComplexity.ft_l1(X,y))

@metafeatures.define("best_node_mean")
class best_node_mean(MetaFeature):
    def _calculate(self, X, y, logger, categorical):
        return np.mean(MFELandmarking.ft_best_node(X,y,score=pymfe.scoring.accuracy))

@metafeatures.define("linear_discr_mean")
class linear_discr_mean(MetaFeature):
    def _calculate(self, X, y, logger, categorical):
        return np.mean(MFELandmarking.ft_linear_discr(X,y,score=pymfe.scoring.accuracy))

@metafeatures.define("leaves_per_class_mean")
class leaves_per_class_mean(MetaFeature):
    def _calculate(self, X, y, logger, categorical):
        return MFE(groups=["model-based"]).fit(X,y).extract()[1][7]

@metafeatures.define("nr_class")
class nr_class(MetaFeature):
    def _calculate(self, X, y, logger, categorical):
        return MFEGeneral.ft_nr_class(X,y)

@metafeatures.define("freq_class_mean")
class freq_class_mean(MetaFeature):
    def _calculate(self, X, y, logger, categorical):
        return np.mean(MFEGeneral.ft_freq_class(X,y))

@metafeatures.define("elite_nn_mean")
class elite_nn_mean(MetaFeature):
    def _calculate(self, X, y, logger, categorical):
        return np.mean(MFELandmarking.ft_elite_nn(X,y,score=pymfe.scoring.accuracy))

@metafeatures.define("conceptvar_mean")
class conceptvar_mean(MetaFeature):
    def _calculate(self, X, y, logger, categorical):
        return np.mean(MFEConcept.ft_conceptvar(X,y))

@metafeatures.define("l2_mean")
class l2_mean(MetaFeature):
    def _calculate(self, X, y, logger, categorical):
        return np.mean(MFEComplexity.ft_l2(X,y))

@metafeatures.define("f1v_mean")
class f1v_mean(MetaFeature):
    def _calculate(self, X, y, logger, categorical):
        return np.mean(MFEComplexity.ft_f1v(X,y))

@metafeatures.define("nre")
class nre(MetaFeature):
    def _calculate(self, X, y, logger, categorical):
        return MFEClustering.ft_nre(X,y)

@metafeatures.define("random_node_mean")
class random_node_mean(MetaFeature):
    def _calculate(self, X, y, logger, categorical):
        return np.mean(MFELandmarking.ft_random_node(X,y,score=pymfe.scoring.accuracy))

@metafeatures.define("worst_node_mean")
class worst_node_mean(MetaFeature):
    def _calculate(self, X, y, logger, categorical):
        return np.mean(MFELandmarking.ft_worst_node(X,y,score=pymfe.scoring.accuracy))

@metafeatures.define("l3_mean")
class l3_mean(MetaFeature):
    def _calculate(self, X, y, logger, categorical):
        return np.mean(MFEComplexity.ft_l3(X,y))

@metafeatures.define("class_ent")
class class_ent(MetaFeature):
    def _calculate(self, X, y, logger, categorical):
        return np.mean(np.log1p(MFEInfoTheory.ft_class_ent(X,y)))
...
# @metafeatures.define("LogNumberOfInstances1",
#                      dependency="NumberOfInstances")
# class LogNumberOfInstances1(MetaFeature):
#     def _calculate(self, X, y, logger, categorical):
#         return np.log(metafeatures.get_value("NumberOfInstances"))+1
